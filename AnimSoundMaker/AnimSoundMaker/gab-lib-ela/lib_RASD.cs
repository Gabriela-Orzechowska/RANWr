...
namespace gablibela
{
    public class lib_RASD
    {
        ...
        public static RASD? TryOpenBRASD(string filePath)
        {
            ...
            BetterBinaryReader r = new(stream);
            if (r.ReadString(4, 0x0) != "RASD") return null;
            RASD _rasd = new();
            _rasd.FilePath = filePath;
            lib_NW4R.Xml_Header header = new()
            {
                CreatorName = "None",
                HostName = "None",
        ...
        {
            BetterBinaryWriter writer = new BetterBinaryWriter(File.Open(filepath, FileMode.Create));
        ...
        private static void WriteSoundEvent(BetterBinaryWriter writer, Event @event, ulong start, out ulong lenght, bool roundTo0x04 = true)
        {
